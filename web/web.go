...
func (s *Server) startTask() {
	...
	go func() {
		...
		// collect traffic stats every 10 seconds, delayed 5 seconds on first start to avoid overlap with xray restart
		s.cron.AddJob("@every 10s", job.NewXrayTrafficJob())
	}()

	// check inbound traffic limits and expiry every 30 seconds
	s.cron.AddJob("@every 30s", job.NewCheckInboundJob())
	// notify traffic stats once a day
...
func (s *Server) Start() (err error) {
	// anonymous function for cleanup on error
	defer func() {
		if err != nil {
			s.Stop()
		}
